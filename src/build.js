...
const promises = colors.map(async color => {
  ...
  try {
    ...
    const dominantColorRGB = isColorlessBlender
      ? [255, 255, 255]
      : await getDominantColor(imagePath)
    const parts = /^(\D*)(\d*)$/.exec(color.code)
    const isBlackGreyOrColorlessBlender = ['', 'C', 'N', 'T', 'W'].includes(
      parts[1]
    )
    color.family = parts[1]
    color.saturation = isBlackGreyOrColorlessBlender ? '' : parts[2].slice(0, 1)
    color.brightness = isBlackGreyOrColorlessBlender
      ? parts[2]
      : parts[2].slice(1)
    ...
    return Promise.reject(
      new Error(`Error "${error.message}" in color ${color.code}`)
    )
  }
})
